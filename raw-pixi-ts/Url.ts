...
export class Url
{
    static regexSeparators = /[\x2E\u3002\uFF0E\uFF61]/g
    static portPattern = /:[0-9]*$/
    static simplePathPattern = /^(\/\/?(?!\/)[^\?\s]*)(\?[^\s]*)?$/
    static protocolPattern = /^([a-z0-9.+-]+:)/i
    static regexNonASCII = /[^\x20-\x7E]/
    ...
    static  hostEndingChars = ['/', '?', '#']

    static delims = ['<', '>', '"', '`', ' ', '\r', '\n', '\t']
    static unwise = ['{', '}', '|', '\\', '^', '`'].concat(Url.delims)
    static autoEscape = ['\''].concat(Url.unwise)
    static nonHostChars = ['%', '/', '?', ';', '#'].concat(Url.autoEscape)
    static hostnamePartPattern = /^[+a-z0-9A-Z_-]{0,63}$/
    static hostnamePartStart = /^([+a-z0-9A-Z_-]{0,63})(.*)$/
    static hostnameMaxLen = 255
    static unsafeProtocol = {
        'javascript': true,
        'javascript:': true
      }
    protocol
	...
	static urlFormat(obj) {
        ...
        if (UtilsSettings.util.isString(obj)) { obj = Url.urlParse(obj); }
    ...
    resolveObject (relative) {
        if (UtilsSettings.util.isString(relative)) {
          var rel = new Url();
        ...
        if (isRelAbs) {
          ...
          result.search = relative.search;
          result.query = relative.query;
        } else if (!UtilsSettings.util.isNullOrUndefined(relative.search)) {
          ...
          result.query = relative.query;
          //to support http.request
          if (!UtilsSettings.util.isNull(result.pathname) || !UtilsSettings.util.isNull(result.search)) {
      ...
      static urlParse(url, parseQueryString = null, slashesDenoteHost = null) {
        if (url && UtilsSettings.util.isObject(url) && url instanceof Url) { return url; }
    ...
    parseHost () {
        var host = this.host;
        var port:any = Url.portPattern.exec(host);
        if (port) {
          port = port[0];
          if (port !== ':') {
    ...
    {
        if (!UtilsSettings.util.isString(url)) {
        ...
        var proto:any = Url.protocolPattern.exec(rest);
        if (proto) {
          proto = proto[0];
        ...
        if (!Url.unsafeProtocol[lowerProto]) {
          ...
          // need to be.
          for (var i = 0, l = <any>Url.autoEscape.length; i < l; i++) {
        ...
        encode (obj, sep = null, eq = null, name = null) {
            ...
            if (!name) { return ''; }
            return encodeURIComponent(<any>this.stringifyPrimitive(name)) + eq +
                   encodeURIComponent(<any>this.stringifyPrimitive(obj));
          };
    ...
    if (this.query &&
      UtilsSettings.util.isObject(this.query) &&
